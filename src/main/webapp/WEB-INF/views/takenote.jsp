...
<script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>
	<div id="wrap">
		...
		<h2 class="left">받은 쪽지함</h2>
				<table class="loginc" style="padding-bottom: 20px; padding-top: 20px;" >
				<c:forEach items="${taview }" var="dto">
			<input type="hidden" id="nonum" name="nonum" value="${dto.nonum }" />
			...
			<tr>
				<td>내용</td>
				<td><textarea cols="20" rows="10" readonly>${dto.content }</textarea>
				</td>
			</tr>
			<tr>
				<td colspan="2">
					<input type="button" class="btn_s" id="submit" onclick="window.open('replysendnote?nonum=${dto.nonum}','','location=no, directories=no,resizable=no,status=no,toolbar=no,menubar=no, width=800,height=600,left=500, top=300, scrollbars=yes');return false" value="답장하기" />
				<input type="button" class="btn_s" onclick="location.href='notedel?nonum=${dto.nonum }'" value="삭제하기" />
				<input type="button" class="btn_s" onclick="location.href='notebox?takenick=${membernick }'" value="취소" />
				</td>
			</tr>
			</c:forEach>
		</table>
