...
<table>
	...
	<tbody>
	<c:forEach items="${free_board }" var="dto">
		<tr>
			<td>${dto.daid }</td>
			<td>
				<c:set value="${dto.daindent }" var="endIndent" />
				<c:forEach begin="1" end="${dto.daindent }" var="cnt">
					&nbsp;
					<c:if test="${cnt eq endIndent }">
						<img src="resources/img/reply.gif" />
					</c:if>
				</c:forEach>
				<a href="content_view?daid=${dto.daid }">${dto.datitle }</a>
			</td>
			<td>${dto.daname }</td>
			<td>${dto.dadate }</td>
			<td>${dto.dahit }</td>
	...
	<tr>
		...
		<td></td>
		<td></td>
		<td><input type="button" id="btn_s" onclick="location.href='write_view'" value="글작성"/></td>
	</tr>
	</tbody>
</table>

<form action="free_board" id="form1" name="form1" method="post">
	
	<c:if test="${pageVo.totPage>1 }">
	<c:if test="${pageVo.page>1 }">
		<a href="free_board?page=1">[처음]</a>  <!-- 어떤페이지라도 무조건 처음페이지로 이동 -->
		<a href="free_board?page=${pageVo.page-1 }">[이전]</a>   <!-- 현재페이지에서 -1 -->
	</c:if>
	...
	<c:choose>
		...
		<c:otherwise>
			<a href="free_board?page=${i }">${i }</a>
		</c:otherwise>
	</c:choose>
	</c:forEach>
	
		<c:if test="${pageVo.totPage>pageVo.page }">
		<a href="free_board?page=${pageVo.page+1 }">[다음]</a>   <!-- 현재페이지에서 +1 -->
		<a href="free_board?page=${pageVo.totPage }">[마지막]</a>  <!-- 어떤페이지라도 무조건 처음페이지로 이동 -->
	</c:if>
</c:if>
	<br /><br />
	<c:choose>
