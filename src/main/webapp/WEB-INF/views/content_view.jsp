...
$(function(){
	...
	var daname =$('#daname').val();
	var membernick =$('#membernick').val();
	if (daname != "" || membernick !="" ) {
		if (daname != membernick) {
			 $("#submit").hide();
...
<h3>자유게시판</h3>
	<form action="content_view2" method="post">
...
<table>
		...
		<tr>
			<td class="left">제목</td>
			<td>
				<input type="text" id="datitle" name="datitle" readonly="${content_view.datitle }" value="${content_view.datitle }" />
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td class="left">내용</td>
			<td>
			<input type="text"  id="dacontent" name="dacontent"readonly="${content_view.dacontent }" style="width: 300px; height:100px; " value="${content_view.dacontent }"  />	
			</td>
		</tr>
		<tr>
			<td colspan="2">
				...
				<input type="button" class="btn_s" onclick="location.href='reply_view?daid=${content_view.daid }'" value="답글달기" />
				
				<%-- <button id="button"><a href="delete?daid=${content_view.daid }">삭제</a></button> &nbsp;&nbsp;
				<a href="reply_view?daid=${content_view.daid }">답변</a> &nbsp;&nbsp; --%>
			</td>	
		</tr>
</table>
	</form>
